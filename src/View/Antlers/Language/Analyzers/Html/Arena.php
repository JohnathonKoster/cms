...
/**
 * Packed storage for an HTML sidecar graph.
 *
 * Node zero is always the document. Tree links are intrusive and stored in
 * packed columns. This avoids allocating a PHP array for every child list and
 * keeps the parser's hot path to direct integer-indexed array access.
 *
 * @internal
 */
class Arena
{
    ...
    /** @var int[] Node kind plus compact element flags. */
    ...
    /**
     * Element-only columns. Unlike the packed tree-link columns these are
     * written only for element ids (slot zero is seeded for the document),
     * which keeps ~two-thirds of the slots from existing at all. Reads for
     * non-element ids are always behind an element kind or open-stack guard.
     *
     * @var array<int, string|null>
     */
    public $name = [0 => null];

    /** @var string[] Opening markup for elements; content for leaf nodes. */
    public $value = [''];

    /** @var array<int, string> Element-only, like the name column. */
    public $closing = [0 => ''];
    ...
    {
        $flags = self::ELEMENT
            | ($selfClosing ? self::SELF_CLOSING : 0)
            | ($synthetic ? self::SYNTHETIC : 0)
            | ($namespace === 'svg' ? self::SVG : 0)
            | ($namespace === 'mathml' ? self::MATHML : 0);
        $id = count($this->kinds);
        $this->kinds[] = $flags;
        $this->parents[] = null;
        $this->firstChildren[] = null;
    ...
    public function namespace($id)
    {
        ...
        return $type & self::MATHML ? 'mathml' : 'html';
    }
