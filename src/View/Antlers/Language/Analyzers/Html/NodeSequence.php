...
/**
 * A reusable lazy view over an ordered sequence of sibling HTML graph nodes.
 */
class NodeSequence
{
    /** @var Document */
    ...
    public static function childrenOf(Document $document, $parent)
    {
        return new self($document, function ($reverse) use ($document, $parent) {
            ...
            $node = $reverse ? $arena->lastChild($parent) : $arena->firstChild($parent);

            while ($node !== null) {
                yield $document->handle($node);
                $node = $reverse ? $arena->previous($node) : $arena->next($node);
            }
        });
    }
    ...
    public static function between(AntlersNode $opening, AntlersNode $closing)
    {
        ...
        return new self($document, function ($reverse) use ($opening, $closing) {
            $boundary = $reverse ? $opening : $closing;
            $node = $reverse ? $closing->previousSibling() : $opening->nextSibling();
    ...
    /**
     * Rewrites an edge and reports whether semantic content ended the walk.
     */
    protected function rewriteEdge($edge, callable $rewrite, ?callable $descend = null)
    {
        ...
        foreach (($this->iterate)($reverse) as $node) {
            ...
            // Opaque markup and source anchors do not contribute character data.
        }
