...
class Element extends Node
{
    ...
    {
        return $this->name !== null && strcasecmp($this->name, $name) === 0;
    }
    ...
    public function attr($name)
    {
        ...
        return array_key_exists($key, $this->changedAttributes)
    ...
    public function hasAttribute($name)
    {
        ...
        return ! isset($this->removedAttributes[$key])
            && (array_key_exists($key, $this->changedAttributes) || array_key_exists($key, $this->attributes));
    }
    ...
    public function setAttribute($name, $value = null)
    {
        ...
        $this->changedAttributeNames[$key] = $this->isHtml() ? $key : $name;
        $this->touchDocument();
    ...
    {
        $classes = preg_split('/\s+/', trim((string) $this->attr('class')));
    ...
    protected function parseAttributes()
    {
        ...
        while ($index < $length) {
            ...
            // A slash only enters the self-closing state. If it is not
            // followed by `>`, HTML reports a parse error and reconsumes the
            // following byte in the before-attribute-name state.
            ...
            // `=` is a parse error before an attribute name, but it still
            // becomes the first byte of that name (for example, `=x`).
            if ($this->openingTag[$index] === '=') {
                $index++;
            }

            while ($index < $length && ! preg_match('/[\x09\x0A\x0C\x0D\x20\/>=]/', $this->openingTag[$index])) {
                if (substr($this->openingTag, $index, 2) === '{{') {
                    $dynamicName = true;
                    $index = $this->antlersEnd($index);
            ...
            if (! array_key_exists($name, $this->attributes)) {
                // HTML keeps the first duplicate attribute and ignores later
                // occurrences during tree construction.
    ...
    protected function escapeAttributeValue($value)
    {
        ...
        while (($start = strpos($value, '{{', $offset)) !== false) {
            $escaped .= htmlspecialchars(
                substr($value, $offset, $start - $offset),
                ENT_QUOTES | ENT_SUBSTITUTE,
                'UTF-8'
            );

            $end = $this->antlersEndIn($value, $start);
            $escaped .= substr($value, $start, $end - $start);
            $offset = $end;
        }

        return $escaped.htmlspecialchars(
            substr($value, $offset),
            ENT_QUOTES | ENT_SUBSTITUTE,
            'UTF-8'
        );
    ...
    /**
     * Returns the offset after the region's `}}`, treating an unterminated
     * region as running to the end of the source.
     */
