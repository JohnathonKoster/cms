...
class IncludeNestedTest extends ParserTestCase
{
    use FakesViews;
    ...
    public function test_an_include_can_render_another_include()
    {
        $this->viewShouldReturnRaw('level1', 'L1{{ include:level2 }}');
        $this->viewShouldReturnRaw('level2', 'L2');

        $this->assertSame('L1L2', $this->render('{{ include:level1 }}'));
    }
    ...
    public function test_deep_assignment_does_not_leak_up_any_level()
    {
        $this->viewShouldReturnRaw('level1', 'L1{{ include:level2 }}[{{ x }}]');
        $this->viewShouldReturnRaw('level2', '{{ x = "deep" }}L2[{{ x }}]');

        $template = '{{ x = "top" }}{{ include:level1 }}[{{ x }}]';

        $this->assertSame('L1L2[deep][][top]', $this->render($template));
    }
    ...
    public function test_cascade_is_not_inherited_by_nested_includes()
    {
        Cascade::set('cval', 'C');
        $this->viewShouldReturnRaw('level1', 'L1[{{ cval }}]{{ include:level2 }}');
        $this->viewShouldReturnRaw('level2', 'L2[{{ cval }}]');

        $this->assertSame('L1[C]L2[]', $this->render('{{ include:level1 cascade="true" }}'));
    }
    ...
    public function test_default_slot_does_not_leak_into_a_nested_include()
    {
        $this->viewShouldReturnRaw('outer', '<o>{{ slot }}</o>{{ include:inner }}{{ /include:inner }}');
        $this->viewShouldReturnRaw('inner', '<i>[{{ slot }}]</i>');

        $template = '{{ include:outer }}OUTER BODY{{ /include:outer }}';

        $this->assertSame('<o>OUTER BODY</o><i>[]</i>', $this->render($template));
    }
    ...
    public function test_include_in_a_loop_has_no_state_bleed()
    {
        $this->viewShouldReturnRaw('row', '<r>{{ n }}</r>');

        $template = '{{ items }}{{ include:row :n="value" }}{{ /items }}';

        $this->assertSame(
            '<r>1</r><r>2</r><r>3</r>',
            $this->render($template, ['items' => [['value' => 1], ['value' => 2], ['value' => 3]]])
        );
    }

    public function test_include_in_a_loop_with_a_slot_per_iteration()
    {
        $this->viewShouldReturnRaw('row', '<r>{{ slot }}-{{ n }}</r>');
    ...
    {
        // Arithmetic in bound params (:depth="depth - 1") isn't evaluated by
        // Antlers, so the decrement uses the subtract modifier.
    ...
    public function test_an_assignment_in_one_include_is_not_seen_by_a_sibling()
    {
        $this->viewShouldReturnRaw('setter', '{{ shared = "x" }}S');
        $this->viewShouldReturnRaw('getter', '[{{ shared }}]');

        $this->assertSame('S[]', $this->render('{{ include:setter }}{{ include:getter }}'));
    }
